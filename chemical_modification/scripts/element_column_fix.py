...
def fix_pdb_element_column(input_file, output_file=None, merge_residues=True):
    """
    ...
    Optionally merges all residue names to a single name for obabel bond perception.
    """
    ...
    # Merge all residue names so obabel perceives inter-segment bonds
    if merge_residues:
        merged_lines = []
        for line in fixed_lines:
            if line.startswith(('ATOM', 'HETATM')):
                # Replace residue name field (columns 17-20) with NAA
                line = line[:17] + 'NAA' + line[20:]
            merged_lines.append(line)
        fixed_lines = merged_lines
        print("Residue names unified to NAA for bond perception.")
