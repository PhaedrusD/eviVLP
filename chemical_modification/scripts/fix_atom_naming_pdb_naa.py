...
def extract_atom_info(str_file_path):
    ...
    # Second pass: extract atom names, skipping any that contain 'X'
    atom_info = []
    for line in updated_lines:
        if line.startswith("ATOM"):
            ...
            if len(parts) >= 2:
                atom_name = parts[1]
                if 'X' not in atom_name:  # <-- filter out X-containing names
...
def update_pdb_file(pdb_file_path, atom_info):
    ...
    # Step 2: For all lines starting with ATOM, replace 4th column with NAA and 6th column with 1
    updated_lines2 = []
    for line in new_lines:
        if line.startswith("ATOM"):
            ...
            new_line = line[:17] + "NAA" + line[20:22] + "   1" + line[26:]
...
def main():
    str_file_path = 'nad.str'
    pdb_file_path = 'nad.pdb'
