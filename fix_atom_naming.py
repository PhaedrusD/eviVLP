...
def extract_atom_info(str_file_path):
    ...
    for line in lines:
        # Update RESI line if it contains *****
        if line.startswith("RESI") and "one.pdb" in line:
            line = line.replace("one.pdb", "ONE")
        updated_lines.append(line)
...
def update_pdb_file(pdb_file_path, atom_info):
    ...
    # Step 2: For all lines starting with ATOM, replace 4th column with ONE and 6th column with 1
    updated_lines2 = []
    for line in new_lines:
        if line.startswith("ATOM"):
            ...
            new_line = line[:17] + "ONE" + line[20:22] + "   1" + line[26:]
...
def main():
    str_file_path = 'one.str'
    pdb_file_path = 'one.pdb'
